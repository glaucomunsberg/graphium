...
class Agent(Thread):
    ...
    def run(self):
        ...
        try:
            ...
            while self._continue_the_job:
                ...
                swarm_cycles = self._swarm_at_mongo['cycles_number']
    ...
    def setAgentName(self):
        ...
        url = self._g.swarm['swarm_agent_names_API']
        response = urllib.urlopen(url)
        try:
            data = json.loads(response.read())
            self.setName(data[0])
        except URLError as e:
            self.setName(self._g.swarm['swarm_agent_names'][random.randint(0, len(self._g.swarm['swarm_agent_names'])-1)])
