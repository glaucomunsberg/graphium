...
class API:
    ...
    def getStreetGeocodeInfoFromMaps(self,lat,lng):
        ...
        url_info = "https://maps.googleapis.com/maps/api/geocode/json?latlng="+str(lat)+","+str(lng)+"&key="+key
        ...
        try:
            ...
            if data['status'] == "OK":
                ...
                self._logger.info("API: Error to get Geocode from point {0},{1} error '{2}'.".format(lat,lng,data['status']))

        except Exception as error:
            self._logger.info("API: Error at Internet to get Geocode from point {0},{1} url '{2}'.".format(lat,lng,url_info))
            return_json['status'] = "ERROR"
        return return_json
    ...
    def getStreetGeoCodeInfoFromOSM(self,way_id):
        ...
        try:
            ...
            if "error" in data.keys():
                ...
                self._logger.info("API: Error to get nominatim from way_id {0} error '{1}'.".format(way_id,data['error']))
